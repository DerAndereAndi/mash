...
func GenerateFeature(def *RawFeatureDef, shared *RawSharedTypes) (string, error) {
...
func GenerateSharedEnums(shared *RawSharedTypes) (string, error) {
	var b strings.Builder
...
func ValidateFeatureIDs(types []RawModelTypeDef, defs []*RawFeatureDef) error {
...
func GenerateFeatureTypes(types []RawModelTypeDef) (string, error) {
	var b strings.Builder
...
func GenerateEndpointTypes(types []RawModelTypeDef) (string, error) {
	var b strings.Builder
...
func generateConstants(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "constants", def)
}

// generateEnums writes enum type definitions with const blocks and String() methods.
func generateEnums(b *strings.Builder, enums []RawEnumDef, prefix string) {
...
func generateArrayStructs(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "arrayStructs", def)
}

// generateStruct writes the feature struct definition.
func generateStruct(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "featureStruct", def)
}

// generateConstructor writes the NewXxx() constructor function.
func generateConstructor(b *strings.Builder, def *RawFeatureDef, shared *RawSharedTypes) {
...
func generateGetters(b *strings.Builder, def *RawFeatureDef) {
	name := def.Name
...
func generateSetters(b *strings.Builder, def *RawFeatureDef) {
	name := def.Name
	recv := strings.ToLower(name[:1])
...
func generateCommandConstants(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "commandConstants", def)
}

func generateCommandStructs(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "commandStructs", def)
}

func generateAddCommands(b *strings.Builder, def *RawFeatureDef) {
	renderTemplate(b, "addCommands", def)
}

func generateCommandHandlers(b *strings.Builder, def *RawFeatureDef) {
	name := def.Name
	recv := strings.ToLower(name[:1])
...
func writeNilHandlerDefault(b *strings.Builder, cmd RawCommandDef) {
...
func writeResponseSerialize(b *strings.Builder, cmd RawCommandDef, recv string) {
...
func writeParamParse(b *strings.Builder, p RawParameterDef, fieldName, structVar string) {
...
func generateCallbackSetters(b *strings.Builder, def *RawFeatureDef) {
...
func isSimpleResponse(cmd RawCommandDef) bool {
...
func commandHandlerType(cmd RawCommandDef) string {
	return typedHandlerType(cmd)
}

func commandFieldType(p RawParameterDef) string {
...
func defaultValueExpr(attr RawAttributeDef, allEnums map[string]RawEnumDef) string {
	if attr.Default == nil {
		return "nil"
	}
...
func enumDefaultConst(attr RawAttributeDef, def *RawFeatureDef) string {
	if attr.Default != nil {
		if defStr, ok := attr.Default.(string); ok {
...
func buildEnumLookup(def *RawFeatureDef, shared *RawSharedTypes) map[string]RawEnumDef {
	lookup := make(map[string]RawEnumDef)
	if shared != nil {
