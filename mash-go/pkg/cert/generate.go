...
// GenerateDeviceID generates a unique device ID for a CSR.
// The device ID is derived from SHA-256(public_key)[:8], giving 64 bits (16 hex chars).
// This is called by the controller to assign a device ID during certificate signing.
func GenerateDeviceID(csrDER []byte) (string, error) {
...
// Deprecated: Use SignCSRWithDeviceID instead - controller should assign device ID.
func SignCSR(ca *ZoneCA, csrDER []byte) (*x509.Certificate, error) {
	// For backward compatibility, use empty device ID which preserves CSR subject
	return SignCSRWithDeviceID(ca, csrDER, "")
}

// SignCSRWithDeviceID signs a Certificate Signing Request with a controller-assigned device ID.
// This follows the Matter pattern where the commissioner assigns the Node ID and embeds it
// in the certificate subject. The device ID becomes the CommonName in the certificate subject.
//
// If deviceID is empty, the CSR's subject is used as-is (backward compatibility).
func SignCSRWithDeviceID(ca *ZoneCA, csrDER []byte, deviceID string) (*x509.Certificate, error) {
	...
	// Build subject - either from CSR or with controller-assigned device ID
	subject := csr.Subject
	if deviceID != "" {
		// Controller-assigned device ID replaces CommonName (Matter-style)
