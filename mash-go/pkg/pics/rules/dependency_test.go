...
func TestDEP002_ASYMMETRICRequiresMultiPhase(t *testing.T) {
	tests := []struct {
		...
		{
			name: "ASYMMETRIC without ELEC - violation",
			input: `MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F09=1`,
			expectViolate: true,
...
func TestDEP003_SIGNALSRequiresSIG(t *testing.T) {
	tests := []struct {
		...
		{
			name: "SIGNALS without SIG - violation",
			input: `MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F04=1`,
			expectViolate: true,
		...
		{
			name: "SIGNALS with SIG - no violation",
			input: `MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F04=1
MASH.S.SIG=1`,
...
func TestDEP005_PLANRequiresPLANFeature(t *testing.T) {
	...
	// Without PLAN feature - violation
	p, _ := pics.ParseString(`MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F06=1`)
	violations := rule.Check(p)
	...
	// With PLAN feature - no violation
	p, _ = pics.ParseString(`MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F06=1
MASH.S.PLAN=1`)
...
MASH.S.ELEC=1`)
	...
	// V2X with ELEC.A05 - no violation (we can't check the actual value)
	p, _ = pics.ParseString(`MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F0A=1
MASH.S.ELEC=1
...
MASH.S.CTRL.F02=1`)
	...
	// Only EMOB - no violation
	p, _ = pics.ParseString(`MASH.S=1
MASH.S.CTRL=1
MASH.S.CTRL.F03=1`)
	violations = rule.Check(p)
	if len(violations) > 0 {
		t.Error("Expected no violation for only EMOB")
	}
}
