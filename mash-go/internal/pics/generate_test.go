...
func TestGenerateUseCaseCodes_DeviceDecls(t *testing.T) {
	decls := []*model.UseCaseDecl{
		{EndpointID: 1, ID: uint16(usecase.GPLID), Major: 1, Minor: 0, Scenarios: 0x01},
	...
	for _, expected := range []string{"MASH.S.UC.EVC", "MASH.S.UC.GPL", "MASH.S.UC.MPD"} {
		if !ucEntries[expected] {
			t.Errorf("missing expected entry %s", expected)
		}
	}
	for _, expected := range []string{"MASH.S.UC.EVC.S00", "MASH.S.UC.GPL.S00", "MASH.S.UC.MPD.S00"} {
		if !ucEntries[expected] {
...
func TestGenerateUseCaseCodes_ControllerDecls(t *testing.T) {
	decls := []*model.UseCaseDecl{
		{EndpointID: 0, ID: uint16(usecase.GPLID), Major: 1, Minor: 0, Scenarios: 0x07}, // BASE + CONSUMPTION + PRODUCTION
		{EndpointID: 0, ID: uint16(usecase.EVCID), Major: 1, Minor: 0, Scenarios: 0x01}, // BASE only
	...
	for _, expected := range []string{"MASH.C.UC.GPL", "MASH.C.UC.EVC", "MASH.C.UC.MPD"} {
		if !ucEntries[expected] {
			t.Errorf("missing expected entry %s", expected)
		}
	}
	// Check GPL scenarios
	for _, expected := range []string{"MASH.C.UC.GPL.S00", "MASH.C.UC.GPL.S01", "MASH.C.UC.GPL.S02"} {
		if !ucEntries[expected] {
			t.Errorf("missing expected scenario entry %s", expected)
		}
	}
...
func TestGenerateUseCaseCodes_Deduplication(t *testing.T) {
	...
	decls := []*model.UseCaseDecl{
		{EndpointID: 1, ID: uint16(usecase.GPLID), Major: 1, Minor: 0, Scenarios: 0x01},
		{EndpointID: 2, ID: uint16(usecase.GPLID), Major: 1, Minor: 0, Scenarios: 0x01},
		{EndpointID: 1, ID: uint16(usecase.MPDID), Major: 1, Minor: 0, Scenarios: 0x01},
	}

	entries := GenerateUseCaseCodes(decls, SideServer)

	// GPL (deduped) + MPD = 2 UCs, each with 1 scenario = 4 entries
	if len(entries) != 4 {
