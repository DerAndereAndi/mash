...
func TestLoadPICS_YAMLFile(t *testing.T) {
	...
	// Check some PICS items (using MASH.* format with endpoints)
	if v, ok := pf.Items["MASH.S.TRANS.SC"]; !ok || v != true {
		t.Error("MASH.S.TRANS.SC should be true")
	}
	if v, ok := pf.Items["MASH.S.E01.ELEC.A01"]; !ok || v != true {
		t.Errorf("MASH.S.E01.ELEC.A01 (phaseCount) should be true, got %v", pf.Items["MASH.S.E01.ELEC.A01"])
	}
	if v, ok := pf.Items["MASH.S.E01.ELEC.A0D"]; !ok || v != true {
		t.Errorf("MASH.S.E01.ELEC.A0D (maxCurrentPerPhase) should be true, got %v", pf.Items["MASH.S.E01.ELEC.A0D"])
	}
}
